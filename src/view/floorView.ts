...
export function initFloorView(scene: THREE.Scene): void {
  slabs = new THREE.InstancedMesh(slabGeometry(), pbrMaterial(textures().floor, { color: activeBiome().floorTint }), MAX);
  slabs.count = 0;
...
export function updateFloorView(game: Game): void {
  ...
  const along = (track: Track, from: number, to: number) => {
    for (let s = from + SLAB / 2; s < to + 1e-6; s += SLAB) {
      if (holed(s)) continue;
      const w = track.sample(s); put(w.x, w.z, w.dir);
    }
  };
  const track = game.track;
  for (const seg of track.segments) {
    const s1 = seg.s0 + seg.length;
    if (seg.kind === 'straight') { along(track, seg.s0, s1); continue; }
    const corner = track.cornerOf(seg);
    along(track, seg.s0, corner - TRACK_HALF_WIDTH);
    ...
    if (seg.fork) forkStubs(seg, c, put);
    if (seg.resolved) along(track, corner + TRACK_HALF_WIDTH, s1);
  }
  slabs.count = n;
